...
import { ExclusiveFeatureSection } from "@/types/global";
import FeatureCard from "../scoped/feature-card";

const ExclusiveFeatures = ({exclusiveFeatures}:{exclusiveFeatures: ExclusiveFeatureSection}) => {
  // 1. State hooks
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 px-5 border divide-y divide-gray-200 rounded-md border-border">
        {
          exclusiveFeatures.values.map((item)=>(
